...
class MainWindowDatabaseMaintenanceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_start_disables_action_and_starts_service(self):
        ...
        window = SimpleNamespace(
            _database_maintenance_service=service,
            actionStartDatabaseMaintenance=action,
            statusBar=lambda: status_bar,
            _on_database_maintenance_failed=Mock(),
        )

        MainWindow.start_database_maintenance(window, options)
    ...
    def test_progress_updates_dialog_and_status_bar(self):
        ...
        status_bar = Mock()
        window = SimpleNamespace(
            _database_maintenance_dialog=dialog,
            statusBar=lambda: status_bar,
        )
        ...
        MainWindow._on_database_maintenance_progress_changed(window, progress)
    ...
    def test_completion_restores_action_and_reports_summary(self):
        ...
        window = SimpleNamespace(
            actionStartDatabaseMaintenance=action,
            statusBar=lambda: status_bar,
            _close_database_maintenance_dialog=close_dialog,
            _database_maintenance_summary=lambda result: (
                MainWindow._database_maintenance_summary(result)
            ),
        )
